...
for req in `find $TICKET_DIRECTORY/test_req.* -type f 2> /dev/null`; do
  ...
  if [[ "$CONDOR_INTEGRATION" == "yes" ]]; then
    echo "universe        = vanilla" >> $TICKET_DIRECTORY"/condor_sc."$ticket
    echo "arguments = --xml-req "$moved" --result-file "$resp" --calc-matrix --calc-roc --prog-file "$test_prog>> $TICKET_DIRECTORY"/condor_sc."$ticket
    ...
    "$OM_BIN_DIR"/om_test --xml-req "$moved" --result "$resp" --calc-matrix --calc-roc --log-file "$log" --prog-file "$test_prog"
  fi  
  exit 1
done
