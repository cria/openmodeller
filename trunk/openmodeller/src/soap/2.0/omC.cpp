...
SOAP_SOURCE_STAMP("@(#) omC.cpp ver 2.8.15 2013-07-03 10:59:01 GMT")
...
{
	(void)soap; /* appease -Wall -Werror */
	this->_om__SerializedAlgorithmType_Model::__any = NULL;
}
...
SOAP_FMAC3 int SOAP_FMAC4 soap_out__om__SerializedAlgorithmType_Model(struct soap *soap, const char *tag, int id, const _om__SerializedAlgorithmType_Model *a, const char *type)
{
	if (soap_element_begin_out(soap, tag, soap_embedded_id(soap, id, a, SOAP_TYPE_om__om__SerializedAlgorithmType_Model), type))
		return soap->error;
	soap_outliteral(soap, "-any", &(a->_om__SerializedAlgorithmType_Model::__any), NULL);
	return soap_element_end_out(soap, tag);
}
	...
	if (soap->body && !*soap->href)
	{
		...
		{	soap->error = SOAP_TAG_MISMATCH;
			if (soap_flag___any1 && (soap->error == SOAP_TAG_MISMATCH || soap->error == SOAP_NO_TAG))
				if (soap_inliteral(soap, "-any", &(a->_om__SerializedAlgorithmType_Model::__any)))
				{	soap_flag___any1--;
					continue;
				}
			if (soap->error == SOAP_TAG_MISMATCH)
