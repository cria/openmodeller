...
/**
 * Load a MaxentModel from a ModelFile object.
 * note: Added by rdg (2008-05-18) to allow custom deserialization by openModeller.
 *
 * @param model MaxentModelFile with all necessary properties loaded
 */
void MaxentModel::load(MaxentModelFile& f) {
    m_pred_map = f.pred_map();
    m_outcome_map = f.outcome_map();
    f.params(m_params, m_n_theta, m_theta);
}
...
/**
 * Save a MaxentModel to a MaxentModelFile object.
 * note: Added by rdg (2008-05-18) to allow custom serialization by openModeller.
 *
 */
MaxentModelFile MaxentModel::save() const {
    if (!m_params)
        throw runtime_error("no model to save (empty model)");
    MaxentModelFile f;
    f.set_pred_map(m_pred_map);
