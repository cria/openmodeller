...
class Tree {
    public:
       ...
       uint16 num_split_nodes() const {return split_nodes_;}
       /// Return the number of terminal nodes
       uint16 num_terminal_nodes() const {return terminal_nodes_;}
       /// Return a tree node given its index
       tree_node* get_node(unsigned int index) const;
       /// Add a node
