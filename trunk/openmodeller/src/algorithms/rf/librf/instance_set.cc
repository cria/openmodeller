...
InstanceSet* InstanceSet::load_csv_and_labels(const string& csv_data,
  ...
  istream sdata( data.rdbuf() );
  ifstream labels(label_file.c_str());
  istream slabels( labels.rdbuf() );
  return new InstanceSet(sdata, slabels, header, delim);
}
InstanceSet* InstanceSet::load_csv_and_labels(istream& sdata,
                                              istream& slabels,
                                              bool header,
                                              const string& delim)
{
...
InstanceSet* InstanceSet::load_unsupervised(const string& csv_data,
                                            ...
                                            const string& delim) {
  ifstream data(csv_data.c_str());
  istream is( data.rdbuf() );
  return new InstanceSet(is, seed, header, delim);
}
 ...
 * @param delim CSV delimiter - defaults to ','
 */
InstanceSet* InstanceSet::load_unsupervised(istream& in,
                                            unsigned int * seed,
                                            bool header,
                                            const string& delim) {
  return new InstanceSet(in, seed, header, delim);
}
 ...
 */
InstanceSet::InstanceSet(istream& sdata, 
            istream& slabels,
            bool header, const string& delim) {
  load_csv(sdata, header, delim);
  load_labels(slabels);
 ...
 */
InstanceSet::InstanceSet(istream& in, unsigned int* seed,
                         bool header, const string& delim) {
  load_csv(in, header, delim);
...
void InstanceSet::write_csv(ostream& out, bool header,
  ...
  for (unsigned int i = 0; i < attributes_[0].size(); ++i) {
    for (unsigned int j = 0; j < num_attributes() - 1; ++j) {
...
void InstanceSet::write_transposed_csv(ostream& out,
  ...
  for (unsigned int j = 0; j < num_attributes(); ++j) {
    for (unsigned int i = 0; i < attributes_[0].size() - 1; ++i) {
      out << attributes_[j][i] << delim;
    }
...
void InstanceSet::load_csv(istream& in, bool header, const string& delim) {
  ...
  while(getline(in, buffer)) {
    vector<string> ary;
    StringUtils::split(buffer, &ary, delim);
    assert(ary.size() == (unsigned int)num_features);
...
InstanceSet::InstanceSet(const InstanceSet& set,
  ...
  for (int i = 0; i < weights.size(); ++i) {
    if (weights[i] == 0) {
      // append instance
      for (unsigned int j = 0; j < set.num_attributes(); ++j) {
